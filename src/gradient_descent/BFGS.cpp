...
bool BFGS::solve() {
  ...
  for (INT i = 0; i < this->lrpara->max_iter_num; i++) {
    ...
    matrix::copy(this->lrmodel->weight_vector, this->ls->step_vector);
    matrix::copy(this->last_gradient_vector, this->lrmodel->gradient_vector);
    matrix::copy(this->last_weight_vector, this->lrmodel->weight_vector);
    this->lrmodel->cal_gradient();
    if (euclid_norm_d(this->gradient_vector_norm, (Vector *)this->lrmodel->gradient_vector) < this->lrpara->epsilon) {
      goto end;
    }
    else {
      ...
      inner_product(this->yt_delta, (Vector *)this->y, (Vector *)this->delta);
      ...
      inner_product(this->dtBd, (Vector *)this->deltaT_B, (Vector *)this->delta);
